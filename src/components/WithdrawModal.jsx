...
export default function WithdrawModal({ isOpen, onClose, onWithdraw, maxAmount }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Call the onWithdraw prop with the amount
      const success = await onWithdraw(amountNum);
    ...
    >
      <div className="w-full max-w-md bg-background rounded-lg shadow-xl border border-border">
        <div className="flex items-center justify-between p-4 border-b border-border">
          <h2 className="text-lg font-semibold">Withdraw PYUSD</h2>
          <button 
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6">
          <div className="space-y-4">
            <div>
              <div className="flex justify-between items-center mb-1">
                ...
                >
                  Max: {parseFloat(maxAmount || '0').toFixed(4)} PYUSD
                </button>
              </div>
          ...
          <div className="mt-6 flex justify-end space-x-3">
            ...
            <Button 
              type="submit" 
              disabled={isLoading || !amount || parseFloat(amount) <= 0 || !maxAmount}
